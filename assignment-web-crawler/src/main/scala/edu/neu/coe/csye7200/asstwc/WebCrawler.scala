...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (a <- ns \\ "a"; href <- a\"@href"; head <- href.head) yield createURL(Some(u), head.toString()) // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (content <- getURLContent(u); mona <- MonadOps.asFuture(getLinks(content))) yield mona // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence(for(us2us <- us2) yield MonadOps.sequence(wget(us2us)))
